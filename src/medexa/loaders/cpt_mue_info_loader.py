...
class CptMueInfoLoader(CptMuePort):
    ...
    def get_mue_value(self, cpt_code: str) -> Optional[int]:
        info = self.get_cpt_info(cpt_code)
        if info and "mue" in info:
            return info["mue"].get("limit")
        return None

    def get_mue_adjudication_indicator(self, cpt_code: str) -> Optional[int]:
        info = self.get_cpt_info(cpt_code)
        if info and "mue" in info:
            return info["mue"].get("adjudication")
        return None

    def get_mue_rationale(self, cpt_code: str) -> Optional[str]:
        info = self.get_cpt_info(cpt_code)
        if info and "mue" in info:
            return info["mue"].get("description")
        return None

    def get_mue_adjudication_level(self, cpt_code: str) -> Optional[str]:
        info = self.get_cpt_info(cpt_code)
        if info and "mue" in info:
            return info["mue"].get("adjudication_level")
        return None
