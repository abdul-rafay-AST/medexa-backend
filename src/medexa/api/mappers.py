...
def recording_state(
    ...
    *,
    ...
    elapsed = max(elapsed_seconds, state.client_elapsed_seconds or 0)
    return c.ApiRecordingState(
        status=recording_status(state),
        elapsed_seconds=elapsed,
        units=units,
        next_unit_at=elapsed + max(0, seconds_to_next_unit),
...
def derive_insights(analysis: ClinicalAnalysis) -> list[ProtocolInsight]:
    ...
    for cpt in analysis.cpt_suggestions:
        insights.append(
            ProtocolInsight(
                ...
                type="detected",
                label=cpt.display_name,
                question=f"Bill {cpt.display_name} ({cpt.code})?",
                description=cpt.reason or cpt.descriptor,
            )
        )
    ...
    for diagnosis in analysis.possible_diagnoses[:3]:
        insights.append(
            ProtocolInsight(
                insight_id=_insight_id("protocol", diagnosis),
                type="protocol",
                label="Clinical impression",
                question=f"Document support for: {diagnosis}?",
                description="Confirm clinical findings and documentation for this impression.",
            )
        )
    return insights
