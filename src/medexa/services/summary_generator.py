...
class BedrockSummaryGenerator:
    ...
    def __init__(
        ...
        self._client = BedrockConverseClient(
            model_id=model_id,
            region_name=region_name or "us-east-1",
        )

    def generate(self, state: SessionState) -> str:
        """Generate a patient-facing summary via Bedrock Converse with rules fallback."""
...
Write the summary directly, no JSON formatting."""
            ...
            summary = self._client.converse(
                system="You write patient-friendly therapy visit summaries.",
                user_message=prompt,
                max_tokens=512,
                temperature=0.4,
            ).strip()
