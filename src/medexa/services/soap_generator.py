...
class BedrockSoapGenerator:
    ...
    def generate(self, state: SessionState) -> SoapNote:
        """Generate a SOAP note via Bedrock Converse with rules-based fallback."""
        if not state.transcript_text:
            return self._fallback.generate(state)

        try:
            prompt = self._build_prompt(state)
            content = self._client.converse(
                system="You are a clinical documentation assistant for therapy.",
                user_message=prompt,
                max_tokens=2048,
                temperature=0.2,
            )
