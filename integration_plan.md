...
The backend is **integration-ready** for the rules-only MVP. The following items
have now been **completed** to make it deployment- and integration-ready
without AWS:

| Status | Change | Why |
|--------|--------|-----|
| ✅ Done | `StartSessionRequest`/`Response` extended with `patient_name`, `mrn`, `therapist_id` echo | Frontend display + debugging via `GET /state` |
| ✅ Done | `GET /sessions?status=all` returns ended/paused sessions too | Dashboard "Recent Transactions" |
| ✅ Done | CORS env accepts comma-separated **or** JSON array | Avoids startup crash on misformatted env |
| ✅ Done | `run_api_server.py` binds `0.0.0.0`, honors `$PORT`, reload off by default | Cloud deploy (Render/Railway/Docker) |
| ✅ Done | `boto3`/`mangum` moved to optional `aws` extra | Base install needs **no AWS** |
| ✅ Done | Dockerfile, Procfile, `.dockerignore`, `.gitignore`, `DEPLOYMENT.md` | One-command non-AWS deploy |
| ✅ Done | Voice wording removed from pause/resume (now "buttons") | Matches button-only product decision |
...
**No backend change is blocking** the first end-to-end demo. See `DEPLOYMENT.md`
for the no-AWS deployment path.
